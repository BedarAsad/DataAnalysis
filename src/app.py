...
import datetime
from utils import prep_uploaded_files, find_col

st.set_page_config(layout="wide", page_title="ARCED - Data Preview", page_icon="📊")

st.title("ARCED / NYU — Data Preview Dashboard")
st.markdown("Interactive preview and analysis using uploaded data files.")

# ------------------------------
# FILE UPLOADER (REPLACES FOLDER LOAD)
# ------------------------------
uploaded_files = st.sidebar.file_uploader(
    "Upload father.xlsx, mother.xlsx or multiple CSV/XLSX files",
    type=["xlsx", "xls", "csv"],
    accept_multiple_files=True
)

if not uploaded_files:
    st.warning("Upload at least one dataset to continue.")
    st.stop()

@st.cache_data
def load_uploaded(uploaded_files):
    return prep_uploaded_files(uploaded_files)

df = load_uploaded(uploaded_files)

if df.empty:
    st.error("Uploaded files are empty or unreadable.")
    st.stop()

# ----------------------------------------------------
# AUTO-COLUMN DETECTION (unchanged)
# ----------------------------------------------------
...
# ----------------------------------------------------
# UI CONTENT BELOW (unchanged — full dashboard)
# ----------------------------------------------------
